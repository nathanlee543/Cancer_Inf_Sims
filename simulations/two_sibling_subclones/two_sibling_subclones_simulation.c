...
double expansion(double b[3], double d[3], double t1, double t2, double t, double delta, double u, int max_clones, FILE *fp)
{
    ...
    for (driver=0; driver<3;driver++)
    {
        ...
        for (; Time < (t2 + t + delta) && num_clones[driver]<max_clones ; )
        {
            ...
            // if we just hit t2+t in the last time step record values
    ...
    if (num_clones[0]<max_clones && num_clones[1]<max_clones && num_clones[2]<max_clones)    
    {
        ...
        #pragma omp critical (write_estimates)
        {
            ...
            fprintf(fp, "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n",r_hat, r1_hat, r2_hat, u_hat, t1_hat, t2_hat, tau1_hat,tau2_hat, m1, m2, m1_obs, m2_obs, subclonal, alpha1, alpha2, beta1, beta2,M1,M2,cells[0],cells[1],cells[2],WBC1[0],WBC1[1],WBC1[2]);
            fclose(fp);
        }
        // free memory
        delete[] Clone;
...
int main(int argc, char *argv[]){
    if (argc < 2){
        ...
        fprintf(fp,"r_hat,r1_hat,r2_hat,u_hat,t1_hat,t2_hat,tau1_hat,tau2_hat,m1,m2,m1_obs,m2_obs,gamma,alpha1,alpha2,beta1,beta2,M1,M2,cells[0],cells[1],cells[2],WBC1[0],WBC1[1],WBC1[2]\n");
        fclose(fp);
