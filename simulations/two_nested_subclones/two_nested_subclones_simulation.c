//
//  two_nested_subclones_simulation.c
//  
...
// Monte Carlo simulation of tumor with 2 nested subclones
...
double expansion(double b[3], double d[3], double t1, double t2, double t, double delta, double u, int max_clones, FILE *fp)
{
    restart_sim:
        ...
        double cells[3];
        double alpha1c, alpha2c, beta0c, beta1c, beta2c;
        ...
        for (driver=0; driver<3;driver++)
        {
            ...
            for (; Time < (t2 + t + delta) && num_clones[driver]<max_clones ; )
            {
                ...
                // this condition will be satisfied during driver = 1 loop
                if (Time >= t2 && t2_yet == 0 && driver == 1)
                {   
                    t2_yet = 1;
                    cells[driver] -= 1.0; 

                    // get number of passengers present in founder type-2 cell
                    // pick which type-1 gets the driver mutation
                    rand=gsl_ran_flat(r,0.0,cells[driver]);
        ...
        if (num_clones[0]<max_clones && num_clones[1]<max_clones && num_clones[2]<max_clones)    
        {
            ...
            u_bottom = (f2-f1)*(beta0c/r_hat + beta1c/r1_hat + beta2c/r2_hat);
            ...
            // only return if it's a macroscopic subclone (if .1 < beta1, beta2c < .9)
            if (beta1c > beta1_min && beta1c < beta1_max && beta2c > beta2_min && beta2c < beta2_max)
            {
                return 1.0;
            }
            else
