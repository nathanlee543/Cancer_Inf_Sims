...
double expansion(double b[2], double d[2], double t1, double t, double delta, double u, int max_clones)
{
    restart_sim:
        ...
        int i,j,k,driver,rep;
        int estimates_length = 15;
        double estimates_out[estimates_length][4];
        ...
        int single_cells[4] = {10,20,100,1000};
        int m_int = 0;

        /*
        int where[max_clones][2];
        double Clone[max_clones][2];
        int ancestor[max_clones][2];
        double surviving_clone[max_clones][2];
        int surviving_ancestor[max_clones][2];
        ...
        */
        ...
        double ut_mle[4] = {0.0,0.0,0.0,0.0};
        double m_obs;
        int clone_ind,seq_ind;
        ...
        // setting the minimum alpha1 to 0.05 will probably eliminate a few runs that would have made it. 
        ...
        for (rep = 0; rep < 4; rep++)
        {       
            double n_k[single_cells[rep]];

            for (k = 0; k < single_cells[rep]; k++)
            {
                rand=gsl_ran_flat(r,0.0,cells[1]);
                
                for(j=0; j<=num_clones[1]; j++)
                {
                    if (rand<Clone[j][1])
                    {
                        seq_ind = j;
            ...
            ut_mle[rep] = ut_mle[rep]/single_cells[rep];
            //printf("ut_mle: %f,\n", ut_mle);
        }

        /*for (i=0; i<=num_clones; i++) cout << i << " " << clone[i] << endl;*/
        for (driver=0; driver<2;driver++)
        {
        ...
        if (num_clones[0]<max_clones && num_clones[1]<max_clones)    
        {
            ...
            // loop over the number of single cells used for single cell sequencing
            for (rep = 0; rep < 4; rep++)
            {
                t_map = (ut_mle[rep] - m_clonal + extra_m_analytic)/u_estimate - delta;
                b_map = r_map*exp(-r_map*(t_map + t1_map + delta))* (1 - alpha2)*M2;
                b1_map = r1_map*exp(-r1_map*(t_map+delta))*alpha2*M2;

                
                estimates_out[0][rep] = r1_map;
                estimates_out[1][rep] = r_map;
                estimates_out[2][rep] = u_estimate;
                estimates_out[3][rep] = t1_map;
                estimates_out[4][rep] = t_map;
                estimates_out[5][rep] = b_map;
                estimates_out[6][rep] = b1_map;
                estimates_out[7][rep] = M1;
                estimates_out[8][rep] = alpha1;
                estimates_out[9][rep] = M2;
                estimates_out[10][rep] = alpha2;
                estimates_out[11][rep] = subclonal;
                estimates_out[12][rep] = (1/r1_map)*log(M1*alpha1) - (0.5772156649/r1_map);
                estimates_out[13][rep] = m;
                estimates_out[14][rep] = m_obs;
            }
            ...
            if (write_mut_freq == 1) 
            {
                #pragma omp critical (mut_freq)
                    ...
                    for (rep = 0; rep < 4; rep++)
                    {
                        ...
                        sprintf(filename, "r1_r_u_t1_t_b_b1_M1_alpha1_M2_alpha2_gamma_%04dcells_f1_p01_f2_p2.txt", single_cells[rep]);
                        
                        myfile.open(filename,fstream::app);  
                        for (j=0; j<estimates_length; j++)
                        {
                            myfile << estimates_out[j][rep] <<" ";
                        }
                        myfile << endl;
                        myfile.close();
                    }    
